...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener, IOpenVPNServiceInternal {
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel,
        ...
        if (mProfile != null)
            nBuilder.setContentTitle("المعهد الإلكتروني");
        else
            nBuilder.setContentTitle("غير متصل");
        ...
        if(status == LEVEL_CONNECTED)
            nBuilder.setSmallIcon(R.drawable.done);
        else if(status == LEVEL_NOTCONNECTED || status ==LEVEL_NONETWORK||status == LEVEL_AUTH_FAILED||status == LEVEL_VPNPAUSED)
            nBuilder.setSmallIcon(R.drawable.close);
        else
            nBuilder.setSmallIcon(R.drawable.reconnect);
    ...
    private void addVpnActionsToNotification(Notification.Builder nbuilder) {
        ...
        nbuilder.addAction(0,getString(R.string.cancel_connection), disconnectPendingIntent);
    ...
    public void updateByteCount(long in, long out, long diffIn, long diffOut) {
        ...
        if (mDisplayBytecount) {
            String s = "↓%2$s %1$s - ↑%4$s %3$s";
            String netstat = String.format(s,
            ...
            showNotification("متصل الآن", null, NOTIFICATION_CHANNEL_BG_ID, mConnecttime, LEVEL_CONNECTED, null);
            byteIn = String.format("↓%2$s", s,
